...
僕が推奨する命名ルールについて「[僕のWeb開発の、変数と関数、ファイル名とフォルダ名の命名ルール](https://outcloud.blogspot.com/2021/08/naming-rules.html) 」を併せて確認ください。
...
| packageGlobal | パッケージに所属するモジュール群で共有するグローバル変数とグローバル関数(*1) |
| module        | パッケージに所属する、コードブロック群の単位 |
| moduleGlobal  | モジュールに所属するコード群で共有するグローバル変数とグローバル関数(*1) |

1. Closure Compiler にはよく似た方向性の [`@packege` 指定](https://github.com/google/closure-compiler/wiki/Annotating-JavaScript-for-the-Closure-Compiler#package)がありました。「パッケージプライベート」という名称でしたので、こちらに寄せるか？迷っています。
